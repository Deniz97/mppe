...
from server.py_rmpe_config import RmpeGlobalConfig, RmpeCocoConfig
from server.py_rmpe_transformer import Transformer, AugmentSelection
from server.py_rmpe_heatmapper import Heatmapper

from time import time

class RawDataGenerator:

    def __init__(self, h5files, configs, augment = True):
        ...
        for n,d in enumerate(self.datums):
            ...
            print(len(k))
            self.total_data_count += len(k)
            self.keys += zip([n] * len(k), k)

    def gen(self, index, timing = False):
        
        num, key = self.keys[index]

        read_start = time()
        image, mask, mask_all, meta, debug = self.read_data(num, key)

        aug_start = time()
        image, mask, mask_all, meta, labels = self.transform_data(image, mask, mask_all, meta)
        image = np.transpose(image, (2, 0, 1))

        if timing:
            return image, mask, mask_all, labels, meta['joints'], time()-read_start, time()-aug_start
        else:
            return image, mask, mask_all, labels, meta['joints']
    ...
    def read_data_old(self, datum, key, config=RmpeCocoConfig):
        ...
        mask_miss = data[:,:,4]
        mask = data[:,:,5]
        # print('miss %s'%str(mask_miss.shape))
        # print('mask %s'%str(mask.shape))
        return img, mask_miss, mask, meta, debug
    ...
    def transform_data(self, img, mask, mask_all, meta):

        aug = AugmentSelection.random() if self.augment else AugmentSelection.unrandom()
        img, mask, mask_all, meta = Transformer.transform(img, mask, mask_all, meta, aug=aug)
        labels = self.heatmapper.create_heatmaps(meta['joints'], mask)

        return img, mask, mask_all, meta, labels
