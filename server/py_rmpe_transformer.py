...
from server.py_rmpe_config import RmpeGlobalConfig, TransformationParams

class AugmentSelection:

    def __init__(self, flip=False, degree = 0., crop = (0,0), scale = 1.):
        self.flip = False # flip
...
class Transformer:
    ...
    @staticmethod
    def transform(img, mask, mask_all, meta, aug=AugmentSelection.random()):
        ...
        # print(img.shape)
        img = cv2.warpAffine(img, M, (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                             flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_CONSTANT, borderValue=(127,127,127))
        mask = cv2.warpAffine(mask, M, (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                              flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_CONSTANT, borderValue=255)
        mask_all = cv2.warpAffine(mask_all, M, (RmpeGlobalConfig.height, RmpeGlobalConfig.width),
                              flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
        mask = cv2.resize(mask, RmpeGlobalConfig.mask_shape, interpolation=cv2.INTER_CUBIC)
        mask_all = cv2.resize(mask_all, RmpeGlobalConfig.mask_shape, interpolation=cv2.INTER_CUBIC)
        ...
        mask_all = mask_all.astype(np.float) / 255.
