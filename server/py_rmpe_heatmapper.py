...
class Heatmapper:
    ...
    def __init__(self, sigma=TransformationParams.sigma, thre=TransformationParams.paf_thre):
        ...
        self.width = RmpeGlobalConfig.width//self.stride
        self.height = RmpeGlobalConfig.height//self.stride

        # this is coordinates of centers of bigger grid
        self.grid_x = np.arange(self.width)*self.stride + self.stride/2-0.5
        self.grid_y = np.arange(self.height)*self.stride + self.stride/2-0.5

        self.Y, self.X = np.mgrid[0:RmpeGlobalConfig.height:self.stride,0:RmpeGlobalConfig.width:self.stride]
    ...
    def create_heatmaps(self, joints, mask):
        ...
        heatmaps = np.zeros((18, self.height, self.width), dtype=np.float)
    ...
    def put_gaussian_maps(self, heatmaps, layer, joints):
        ...
        for i in range(joints.shape[0]):
            ...
            heatmaps[layer, :, :] = np.maximum(heatmaps[layer, :, :], exp)
